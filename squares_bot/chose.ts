...
export class Chosé {
     ...
     */
    public getBestMove(move: Move, defaultDepth = 15): Move | undefined {
     ...
     * @returns 
     */
    private runThroughAndAwardChildren(parentMove: ChainedMove, depth: number) {
        ...
        for (const child of children) {
            if (parentMove.childMoves[0]) {
                console.log("has child");
                if (parentMove.childMoves[0].status == (parentMove.playedBy == Player.second ? 1 : -1)) {
